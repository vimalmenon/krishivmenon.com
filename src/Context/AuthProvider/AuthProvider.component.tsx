...
import { useLocalStorage } from './AuthProvider.hook';
...
export const AuthProvider: React.FC<ReactChildren> = ({ children }) => {
  const [accessToken] = React.useState<string | null>(initialValue.accessToken);
  const [idToken] = React.useState<string | null>(initialValue.idToken);
  useLocalStorage();
  React.useEffect(() => {
